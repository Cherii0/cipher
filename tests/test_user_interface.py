...
def test_when_given_filepaths_and_input_correct_then_return_filepath(
    mocker, monkeypatch
):
    expected_filepath = "file1.txt"
    mocker.patch("os.listdir", return_value=["file1.txt", "file2.txt"])
    monkeypatch.setattr(
        target=builtins, name="input", value=lambda _: expected_filepath
    )
...
@pytest.mark.choice_filepath
def test_when_correct_filepath_and_given_incorrect_filepaths_then_correct_then_return_filepath(
    mocker, monkeypatch
):

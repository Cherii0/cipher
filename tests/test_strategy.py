import sys
sys.path.append('../src')
from src.strategy import CipherFactory, EmptyRotMethods, ROT13Strategy, UserCancelReplace
import pytest
...
@pytest.mark.strategy_rot13
def test_validate_content_when_incorrect_and_no_replace_then_raise(create_rot13_cipher, mock_replace_option_false):
    content = "ab cźd"
    with pytest.raises(UserCancelReplace):
        create_rot13_cipher.validate_content(content)

@pytest.mark.strategy_rot13
def test_validate_content_when_incorrect_and_replace_then_pass(create_rot13_cipher, mock_replace_option_true):
    content = "ab cźd"
...
def test_validate_content_when_no_content_then_raise(create_rot47_cipher):
    content = ""
    with pytest.raises(ValueError):
...
@pytest.mark.strategy_rot47
def test_validate_content_when_incorrect_and_no_replace_then_raise(create_rot47_cipher, mock_replace_option_false):
    content = "ab cd"
    with pytest.raises(UserCancelReplace):
        create_rot47_cipher.validate_content(content)

@pytest.mark.strategy_rot47
def test_validate_content_when_incorrect_and_replace_then_correct(create_rot47_cipher, mock_replace_option_true):
    content = "ab cd"

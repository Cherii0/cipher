...
def init_cipher_obj(mocker):
    mocker.patch(
        target="src.user_interface.UserInterface.show_method_choice",
        return_value=["rot13", "rot47"],
    )
    mocker.patch(
        target="src.user_interface.UserInterface.method_choice", return_value="rot13"
    )
    mocker.patch(
        target="src.strategy.CipherFactory.get_cipher", return_value=ROT13Strategy()
    )
...
def mocker_filepath(mocker):
    mocker.patch(
        target="src.user_interface.UserInterface.choice_filepath",
        return_value="cipher.txt",
    )
    mocker.patch(target="src.file_handler.FileHandler.read", return_value="python")
...
def test_if_execute_from_user_input_return_expected_values_then_pass(
    ...
    mocker.patch(target="src.strategy.ROT13Strategy.execute", return_value="clguba")
